...
## 2026-09-02 - Dynamic Live Region Feedback & Re-click Timer Management
**Learning:** Transient status updates on interactive buttons (such as copying a link) often fail screen reader notification if the label container lacks live region semantics (`aria-live="polite"`). Furthermore, rapid re-clicks on non-debounced feedback buttons can corrupt saved label state strings or trigger race conditions between overlapping timeout callbacks.
**Action:** Dynamically assign `aria-live="polite"` to status label containers during state changes and remove the attribute on reset. Cache original label strings directly on the element (`btn._originalText`) and cancel active timeouts (`clearTimeout(btn._copyTimeout)`) on re-click.
...
## 2026-09-12 - Demoting a Nav Section Requires the Same Cross-File Replication as Adding One
**Learning:** Demoting Instagram/YouTube from a dedicated homepage `#social` section (own heading, nav entry, CTA-style buttons) down to plain footer links was scoped as an `index.html`-only change, but CLAUDE.md's "shared UI must be replicated across all files manually" rule applies equally to *removing/shrinking* a UI element as to adding one — the fix initially shipped with the new footer links on `index.html` only, leaving all 19 other pages (every blog post, `blog/_TEMPLATE.html`, `privacy.html`) with the old 4-link footer and no way to reach those channels at all. A follow-up code review caught it.
**Action:** Any footer/nav change — add, remove, or demote — is a full-site change on this static, template-less codebase. Grep for the exact anchor text/markup being changed (e.g. `id="cookie-prefs-link"`) across `blog/*.html` and `privacy.html` before considering the change done, and always update `blog/_TEMPLATE.html` too so future posts don't regenerate the stale pattern. When scripting a repo-wide text patch, check line endings first (`file <path>`) — this repo's blog files are CRLF, and a naive `\n`-only regex silently matches zero files instead of erroring loudly.

## 2026-09-09 - Modal Accessibility: ARIA Expanded State & Initial Action Focus
**Learning:** When dynamic modal dialogs (such as cookie preference banners) are opened, focusing a static container `div` fails to announce interactive controls and causes the keyboard focus trap to break on the initial Tab keypress because the container element is not part of the focusable elements array. Furthermore, failing to update `aria-expanded` state on the trigger button leaves screen reader users unaware of the dialog's visibility state.
**Action:** Always set `aria-expanded="true"`/`"false"` on modal trigger elements upon open and close, and place initial focus directly on a primary action button inside the modal dialog (`acceptBtn.focus()`) to guarantee an airtight focus trap and immediate control announcement.
