...
(function () {
  ...
  function computeScore(lines, combo, fullBoard) {
    ...
    var total = Math.round(bonus * multiplier);
    if (fullBoard) total += 500;
    return { total: total, bonus: bonus, multiplier: multiplier, voiceCue: VOICE_CUE[comboKey] };
  }
  ...
  function initCalculators() {
    ...
    widgets.forEach(function (widget) {
      ...
      function update() {
        ...
        var result = computeScore(lines, combo, fullBoard);
        ...
        if (breakdownEl) {
          var parts = result.bonus + ' bonus × ' + result.multiplier.toFixed(1) + '× combo';
